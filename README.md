...
3. **Execute `<planning>`:** Open a `<planning>` block to extract facts from the PRD and map out the template headings. 
4. **Draft the Markdown:** Generate the documents using ONLY facts from the PRD, formatting from the `templates/`, and tone from `context/writing_style_guide.md`.
5. **Handle Gaps:** If the PRD is missing information required by a template, do not invent it. Use the exact flag: `> ⚠️ **[CLARIFICATION NEEDED]:** <Describe gap>`.
6. **Execute `<self_audit>`:** Run the checks defined in `agent_review_checklist.md` inside a `<self_audit>` block. Correct any `[FAIL]` states before finalizing your output.

---

<strict_warning>
**DO NOT HALLUCINATE.** If the PRD does not explicitly state a permission, a limit, or a UI location, you must not invent one to make the document look complete. Flag it as a gap. Your primary directive is factual accuracy over fluency.
</strict_warning>
